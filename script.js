...
veryOldInventors = inventors.filter(inv => (inv.year >= 1500 && inv.year <= 1599));
...
inventorNames = inventors.map(inv => {
...
sortedByBirthYear = inventors.sort(inv => inv.year);
...
inventorNamedAda = inventors.find(el => el.first === 'Ada');
...
firstLast = people.map(inv => {
    const name = inv.split(', ');
...
isAdultPresent = devs.some(person => (2024 - person.year) >= 18 );
...
isEveryone19OrOlder = devs.every(person => (2024 - person.year >= 19));
...
totalYearsLived = inventors.reduce((accumulator, currentVal) => {
    
    let age = currentVal.passed - currentVal.year;
...
travelMethodCounts = travelMethods.reduce((acc, travelMethod) => {
    if(acc[travelMethod])
        acc[travelMethod] = acc[travelMethod] + 1;
    else
        acc[travelMethod] = 1;
